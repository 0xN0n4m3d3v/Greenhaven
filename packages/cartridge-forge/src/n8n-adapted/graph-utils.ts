// Copied from n8n `packages/workflow/src/graph/graph-utils.ts` and adapted
// to Forge's small workflow shape. Keep this file close to the donor structure
// so future n8n graph utilities can be merged by copy/adapt instead of
// rewriting the algorithm from scratch.
import type {ForgeWorkflow} from '../core/workflowTypes.js';
...
export type IConnections = Record<string, Record<string, IConnection[][]>>;
...
export function buildForgeConnections(workflow: ForgeWorkflow): IConnections {
...
export function validateForgeWorkflowGraph(workflow: ForgeWorkflow): string[] {
	const nodeIds = new Set(workflow.nodes.map(node => node.id));
	const connections = buildForgeConnections(workflow);
	const adjacency = buildAdjacencyList(connections);
	const errors: string[] = [];

	for (const edge of workflow.edges) {
		if (!nodeIds.has(edge.from)) errors.push(`edge source not found: ${edge.from}`);
		if (!nodeIds.has(edge.to)) errors.push(`edge target not found: ${edge.to}`);
	}

	const roots = getRootNodes(nodeIds, adjacency);
	if (roots.size === 0 && workflow.nodes.length > 0) {
		errors.push('workflow has no root node');
	}

	const leaves = getLeafNodes(nodeIds, adjacency);
	if (leaves.size === 0 && workflow.nodes.length > 0) {
		errors.push('workflow has no leaf node');
	}

	for (const node of workflow.nodes) {
		for (const input of node.inputs ?? []) {
			if (!nodeIds.has(input)) errors.push(`node ${node.id} references missing input ${input}`);
			else if (!hasPath(input, node.id, adjacency)) {
				errors.push(`node ${node.id} input ${input} is not connected by workflow edges`);
			}
		}
	}
